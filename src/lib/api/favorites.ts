...
export async function addFavorite(boxId: string): Promise<void> {
  ...
  if (!user) return
  await supabase.from('bookmarks').insert({ user_id: user.id, box_id: boxId })
}
...
export async function removeFavorite(boxId: string): Promise<void> {
  ...
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return
  await supabase.from('bookmarks').delete().eq('user_id', user.id).eq('box_id', boxId)
}

export async function getMyFavoriteBoxIds(): Promise<string[]> {
  const supabase = createClient()
  const { data } = await supabase.from('bookmarks').select('box_id')
