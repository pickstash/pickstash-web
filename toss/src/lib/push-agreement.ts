...
// ⚠️ 반드시 사용자가 직접 클릭한 액션(알림 설정 화면의 "알림 받기" 버튼)에서만 호출할 것 — 앱
// 진입·로그인 직후는 물론, 탭 이동 같은 "클릭이 아닌 자동 트리거"로 걸어도 "미니앱 접속 직후
// 바텀시트 노출" 다크패턴으로 반려된다(여러 차례 반복 확인됨). 오직 명시적 버튼 클릭에서만 호출.
