...
**홈 서랍 레일(§7-1)의 상자 카드**는 위 규칙의 축약판(단순화된 리스트라 정보 밀도를 낮춤):
모드 칩·즐겨찾기 별·좋아요/댓글 수·"정리됨" 배지는 표시하지 않는다(2026-08 리디자인 확정). 대신:
- D-day 배지는 마감 투표 상자에만.
- 결정형: 인기 리더 있으면 인기 배지+이름, 없으면 **"선택지 N개"**(옵션 총개수 — 즐겨찾기·인기와 별개로
  항상 계산됨, 리더 없을 때의 허전함 방지용).
- 체크형: "N/M 체크" 텍스트만(진행바 없음 — 앱 전체에 진행바 선례 없음).
- 정리완료 + 결정 있음: 위 형광펜 강조 그대로. **정리완료인데 결정 없음**(마감 투표가 신호 없이 닫힌 경우,
  §3-3)은 배지 없이 옅은 텍스트 "결정 없이 마감"만 — 배지를 다시 넣지 않으면서도 "리더 없는 열린 상자"와
  구분되게 하는 최소 신호.
...
- **모델 (단일 공유 객체 + 멤버, v2 재설계 2026-07-30 — 노션식)**: 폴더는 **복사본 없는 하나의 공유 객체**다. `folders(id, name, invite_code)` — **오너(`user_id`) 없음**(011 방장 제거와 정합). 누가 멤버인지는 `folder_members(folder_id, user_id, sort)`, 폴더의 상자 목록은 **폴더 스코프 공유** `box_folders(folder_id, box_id, sort)`(user_id 없음 — 멤버 전원이 같은 목록을 본다). **개인 폴더 = 멤버 1명(나)**, **공유 폴더 = 멤버 2명 이상**. 한 상자는 여러 폴더에 들어갈 수 있다. 같은 상자를 나는 "여행" 폴더에, 친구는 "모임" 폴더에 따로 담아도 별개 객체라 안 부딪힌다.
...
실사용 관찰: 참여 인원과 무관하게(혼자든 여럿이든) 투표·결정 없이 **항목을 적고 체크만** 하는 용도(쇼핑리스트 등)로 상자를 쓰는 경우가 있다. **목적(결정형/체크형)과 참여 인원(혼자/여럿)은 서로 다른 축**이다 — 혼자 상자도 결정형을 고를 수 있고(예: 혼자 고민 정리), 여럿 상자도 체크형을 고를 수 있다(예: 같이 짐 싸기). 기존 "혼자 상자는 좋아요 미표시"(§3-2) 로직은 그대로 두고, 그 위에 목적 축을 독립적으로 얹는다.

- **`boxes.mode`**: `'decide'`(기본, 기존 전체 동작) | `'checklist'`(신규). **상자 생성 시 고른 뒤 고정** — 이후 변경 불가(바꾸고 싶으면 새 상자). 기존 상자는 전부 `'decide'`로 백필돼 동작이 그대로 유지된다.
- **아이템 = 선택지**: 체크형 상자의 "항목"은 별도 모델이 아니라 **선택지(`options`) 그대로**다 — 사진 첨부(선택지는 이미 사진 블록 지원)가 핵심 요구사항이라, 텍스트 안 체크박스 서식(019)만으로는 부족해서다. 아이템마다 별도 행이라 **참여자 여럿이 서로 다른 항목을 동시에 체크해도 경합이 없다**(별도 잠금 RPC 불필요, 기존 "options: 참여자 수정" RLS로 충분).
- **`options.checked_at`**: `decided_at`과 같은 결의 파생 상태 — null=미체크, 값=체크됨. 체크형 상자에서만 의미 있다. 체크는 참여자 누구나, 정리완료 상자에서도 가능(§3-4와 동일 원칙).
- **`options.group_label`**: 선택지에 붙는 **선택적** 카테고리 텍스트(예: "면세점"). 없으면 flat 목록, 있으면 그룹으로 묶어 렌더. **강제 구조가 아니다** — 폴더(§3-7, 상자 여러 개를 묶는 범용 도구)와 무관하며, 체크형 카테고리 전용 용도로 재정의하지 않는다.
- **체크형 상자에서 숨는 것**: 좋아요·"1위" 뱃지·"이걸로 정하기"/"최종 결정하기"·마감 투표·결정 방식 변경 메뉴 — 전부 결정형 전용 개념이라 UI에서 아예 노출되지 않는다.
- **"정리완료" 의미가 다르다**: 결정형은 "결정 확정", 체크형은 **"이 리스트는 다 썼음"**(수동 표시, "정리완료로 표시" 버튼) — 둘 다 동일한 `closed_at` 파생 모델(§3-1)을 그대로 재사용한다.
- **초대 링크 뷰어(§6-1)도 동일 반영**: `get_box_view_by_invite_code`가 `mode`·`checked_at`·`group_label`을 함께 내려줘, 비로그인 열람에서도 체크형 상자가 체크박스·그룹으로 보인다(읽기 전용, 클릭 불가).
...
create table boxes (
  ...
  mode text not null default 'decide',   -- 'decide'(결정형, 기본) | 'checklist'(체크형). 생성 시 고정, 변경 불가. 033/§3-8
  deadline_at timestamptz,               -- auto_deadline에서만 사용. manual·checklist는 null(마감 없음)
...
create table options (
  ...
  checked_at timestamptz,                -- null=미체크, 값=체크됨. checklist 상자(mode='checklist')에서만 의미. 033/§3-8
  group_label text,                      -- null=그룹 없음(flat). 있으면 그 텍스트로 묶어 렌더. 강제 아님. 033/§3-8
...
2. Supabase Realtime으로 카운트 실시간 반영 + **"인기: ○○" 실시간 표시.** TanStack Query 낙관적 업데이트 병행.
   - **표시 규칙(2026-08-10)**: 좋아요 최다가 **단독 + 2개 이상**일 때만 '인기'로 강조한다. **동점·1개·무투표는 미표시**(좋아요 1개로 즉시 크라운되거나 공동 N개가 우르르 잡히는 노이즈 방지). 라벨은 경쟁적인 "1위"가 아니라 **"인기"**. 임계값은 `src/lib/domain/winner.ts`의 `MIN_LEADER_LIKES`. (결정 시트의 미리선택은 관대하게 최다 전부를 유지 — 표시용과 별개.)
...
> - **상자 생성**: **목적 선택**(결정하기 / 체크하기, §3-8) 카드가 먼저. "결정하기"를 고르면 기존 **결정 방식 선택**(직접 정하기 / 마감 투표) 카드가 이어진다. 마감일시는 마감 투표를 고를 때만 노출.
> - **상자 상세**: 재투표·시간만료 UI 없음. **결정형**: "이걸로 정하기"(선택지 1+, 참여자 누구나) + 정리완료 후 "다시 정리하기"(번복). 여럿 상자 "지금 1위" 실시간, 혼자 상자 좋아요 미표시. **체크형(§3-8)**: 좋아요·1위·결정 UI 없이 항목마다 체크박스 + 선택적 그룹 헤더, "정리완료로 표시"로 수동 마무리. 둘 다 정리완료여도 편집 가능. 편집 메뉴는 결정형만 결정 방식 변경 노출(체크형은 숨김) + 폴더 지정·나가기는 공통.
...
### 7-1. 메인 `/` (2026-08 리디자인 — 서랍 레일)
- 헤더: "{닉네임}님의 결정창고" + 햄버거 메뉴
- **① 마감 임박 히어로**(있을 때만): 마감 투표(`decision_mode='auto_deadline'`) + **D-3 이내**인 정리중 상자 1개만 크게.
  그 외엔 히어로 없이 바로 아래 서랍 레일의 "전체"에 섞여 보인다("진짜 임박한 것만"— D-3보다 먼 마감이 매번
  히어로 자리를 차지하지 않게). 인기 리더/좋아요·댓글 없이 D-day 배지(긴급도 색)·참여자 아바타·"정하러 가기" CTA.
- **② 서랍 레일**: 가로 스크롤 칩. **"전체" 칩이 항상 첫 자리로 고정**(서랍 유무와 무관하게 홈이 안 비도록)되고
  기본 선택. 그 뒤로 서랍별 칩, 맨 끝에 점선 "+ 새 서랍"(`/folders`로 이동, 생성은 그 화면에서).
  칩을 누르면 **레일 바로 아래에 그 서랍의 상자가 스태거 애니메이션으로 등장**(별도 배경 래핑·"서랍" 제목 없음).
  "전체" 선택 시엔 최근 활동순 최대 5개 + 넘치면 우측에 "전체 보기"(→ `/boxes`); 서랍 선택 시엔 그 서랍 상자
  전부(캡 없음). 서랍이 0개인 유저도 이 레일이 그대로 뜨고 칩이 "전체"+"새 서랍"만 남는다(별도 빈 상태 없음).
  카드 라벨 규칙은 §3-6.
- 하단 고정: "새 상자" FAB
- 첫 사용자(상자·서랍 0개)면 캐릭터 빈 상태 + 안내

### 7-2. 상자 생성 `/box/new`
- 상자 이름 input (필수, 빈 값 검증)
- 메모 input (선택)
- **목적 선택** (`boxes.mode`, §3-8) — 2카드, 생성 후 **변경 불가**:
    - **결정하기** (기본) — 부제: "후보를 모아 투표하거나 골라서 정해요". 고르면 아래 결정 방식 선택이 이어서 노출.
    - **체크하기** — 부제: "항목을 적고 같이 체크만 해요. 투표 없음". 고르면 결정 방식 선택은 숨김(마감·decision_mode는 의미 없어 서버에서 기본값 고정).
- **결정 방식 선택** (`decision_mode`, 목적이 "결정하기"일 때만 노출):
...
### 7-3. 상자 상세 `/box/[id]`
> 상태는 2종(OPEN/RESOLVED)뿐 — EXPIRED 없음. 모든 편집 액션은 **참여자 누구나**(방장 없음, 008/011). 편집 메뉴(⋯)에 제목·메모·결정방식(결정형만)·마감·폴더 지정·나가기를 모은다.
- 헤더: 뒤로가기(**내비 스택 유지** — 진입한 창고로 복귀), 제목, 즐겨찾기 토글
- 정리상태 라벨 + 상자 제목 + 메모 + 편집 메뉴(제목·메모 수정)
- 마감일(`auto_deadline`만 표시, `manual`·체크형은 마감 없음) + "변경하기"(바텀시트) / 결정 방식 변경(결정형·정리중일 때만)
- 참여 인원 수 + 아바타 목록 + "+친구초대" → `/box/[id]/invite`. **참여자가 나 혼자면 초대 유도를 강요하지 않는다** (혼자 모드)
- **선택지/항목 N개 목록**: 카드마다 이름·본문 미리보기(첫 사진 + 첫 글 스니펫). **결정형**: (여럿 상자만)좋아요 버튼+카운트·"1위" 뱃지·"결정" 뱃지, 정렬 토글(최신순/좋아요순). **체크형(§3-8)**: 체크박스(탭하면 즉시 토글, 취소선)·그룹 라벨 있으면 그룹 헤더로 묶어 표시(정렬 토글 없음, 그룹→등록순 고정). 둘 다 무한스크롤, 탭하면 상세로.
- 하단 버튼: **결정형** "선택지 추가하기" / `manual`·정리중이면 "이걸로 정하기"(선택지 1+ 선택→확정, 참여자 누구나). **체크형** "항목 추가하기" / 정리중이면 "정리완료로 표시"(선택 없이 `decide_box([])` 호출 — 결정 없이 마감과 동일 경로 재사용).
- **정리완료(RESOLVED)**: **결정형** 결정 결과("○○(으)로 결정!", 없으면 "결정 없이 마무리") + 스탬프 + "다시 정리하기"(번복, 누구나). **체크형**은 "체크리스트를 정리했어요" + 동일 스탬프·번복. **정리완료여도 제목·메모·선택지·댓글·좋아요·폴더 편집은 계속 가능**(§3-4). 결정형만 투표(좋아요)가 UI에서 비활성("정리된 상자에선 투표할 수 없어요").
...
- (여럿 상자만, 결정형) 좋아요 버튼 + 카운트. 정리완료 상자는 좋아요 비활성. 혼자 상자는 좋아요 미표시.
- **체크형(§3-8)**: 좋아요 대신 이름 옆 체크박스(탭하면 즉시 토글, `checked_at`) + `group_label` 있으면 상단에 그룹 칩. 정리완료 상자에서도 체크 가능(§3-4).
...
- 선택지 이름 input (필수)
- **그룹 input (선택, 체크형 상자만, §3-8)**: 텍스트 입력 + 이 상자에서 이미 쓰인 그룹을 칩으로 자동완성(탭하면 채움). 비워두면 `group_label = null`(flat 목록).
- 본문 블록 에디터: `＋글`·`＋사진`·`＋링크`로 블록 추가, 블록별 ↑↓ 이동·삭제. 사진은 1블록 1장(최대 6장), 링크는 라벨+URL. 상한 20블록. 저장 시 빈 글/빈 URL 블록은 제거
- **글 블록 서식(019, 실시간 최소 서식)**: 글 블록은 Tiptap(contentEditable) 기반 실시간 리치텍스트 에디터 — 타이핑하는 즉시 굵게(B)·기울임(I)·글머리 목록(•)·체크박스(☑)가 반영된다(애플 메모장 방식). 저장 형식은 ProseMirror 문서를 JSON 직렬화한 문자열 — DB 마이그레이션 없이 기존 `content` jsonb의 글 블록 `text` 필드 그대로 사용(레거시 평문 데이터는 파싱 시 문단으로 폴백해 계속 보임). 체크박스는 상자 상세/선택지 상세·초대 미리보기 어디서든 렌더되며, **선택지 상세에서 체크 클릭 시 즉시 저장**(참여자 누구나, §3-4에 따라 정리완료 상자에서도 가능). 초대 미리보기(비로그인)는 읽기 전용(클릭 불가). 읽기 전용 렌더는 Tiptap을 다시 마운트하지 않고 ProseMirror JSON을 직접 순회하는 경량 렌더러(`RichText`)를 쓴다.
  - **배포 노트**: 앱인토스(`toss/`) `ait build`는 `package.json` 직접 의존성 전체의 버전을 수집하는 내부 단계가 있는데, `@tiptap/pm`은 `exports`에 루트(`.`) 항목이 없어 이 단계가 실패한다. `toss/package.json`에서 `@tiptap/pm`을 직접 의존성에서 뺀다(다른 tiptap 패키지의 전이 의존성으로 node_modules엔 그대로 남음) — 코드는 그대로, 직접 import도 안 하므로 안전.
