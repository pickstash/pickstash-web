...
**⚠️ 진행 중 (2026-07-22, 아직 push 안 됨)**: ① 선택지 정렬(최신/좋아요순) + 무한스크롤, ② 선택지 본문 **블록-라이트 모델**(summary/memo/images/links 4필드 → `content` 블록 배열: `{type:'text'|'image'|'link', ...}`) 구현 완료·로컬 커밋됨. **`006_option_content.sql`을 Supabase 대시보드 SQL Editor에 먼저 적용한 뒤 push**할 것. 006은 원본 4컬럼 데이터를 content로 이관하며 **비우므로**(재실행 안전 목적), (a) 실행 전 options 테이블 백업 권장, (b) 적용 직후 곧바로 push해 구(舊) 코드가 빈 컬럼을 읽어 본문이 잠깐 비어 보이는 공백을 최소화. 코드를 먼저 push하면 선택지 생성이 깨짐(content 컬럼 부재).
